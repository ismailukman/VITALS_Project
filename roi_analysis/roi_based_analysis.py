...
OUTPUT_DIR = project_root / "roi_analysis" / "output"
PLOT_DIR = project_root / "roi_analysis" / "output" / "plots"
...
def plot_roi_density_grid(results_df, null_results, all_subjects_data, 
    ...
    With only 3 subjects, uses strip/scatter points + mean lines instead of 
    pure KDE (which needs more data points for smoothing).
    """
    # Compute per-ROI mean PLV delta to rank ROIs
    roi_stats = results_df.groupby(['roi_name', 'roi_index']).agg({
        'plv_delta': 'mean',
        'awplv_delta': 'mean',
        'p_fdr_plv': 'min',
        'p_fdr_awplv': 'min',
        'plv_empirical': list,
        'awplv_empirical': list,
    }).reset_index()
    
    roi_stats = roi_stats.sort_values('plv_delta', ascending=False).head(n_top)
    n_rois_to_plot = len(roi_stats)
    
    if n_rois_to_plot == 0:
        print("  No ROIs to plot.")
        return
    
    # Collect null distributions per ROI
    roi_null_data = {}
    for _, row in roi_stats.iterrows():
        ...
        for (subject, run), null in null_results.items():
            subj_valid_rois = all_subjects_data[(subject, run)]['valid_rois']
            if roi_idx in subj_valid_rois:
                i = subj_valid_rois.index(roi_idx)
                null_plvs_all.extend(null['null_plvs'][:, i].tolist())
                null_awplvs_all.extend(null['null_awplvs'][:, i].tolist())
        
        roi_null_data[roi_name] = {
            'PLV': np.array(null_plvs_all),
            'awPLV': np.array(null_awplvs_all)
        }
    
    # Determine grid dimensions
    n_cols = 4
    n_rows = int(np.ceil(n_rois_to_plot / n_cols))
    ...
    fig.suptitle('EGG-Brain Synchrony: Top ROIs (PLV & awPLV)\n'
                 'Empirical (filled) vs Null/Mismatch (dashed)',
                 fontsize=22, weight='bold', y=0.98)
    
    for idx, (_, row) in enumerate(roi_stats.iterrows()):
        r = idx // n_cols
        c = idx % n_cols
        roi_name = row['roi_name']
        
        # Empirical values (list of per-subject values)
        emp_plv = np.array(row['plv_empirical'])
        emp_awplv = np.array(row['awplv_empirical'])
        
        # Null values
        null_plv = roi_null_data[roi_name]['PLV']
        null_awplv = roi_null_data[roi_name]['awPLV']
        
        # Get min FDR p-value across subjects for annotation
        roi_results = results_df[results_df['roi_name'] == roi_name]
        plv_pvals = roi_results['p_fdr_plv'].values
        awplv_pvals = roi_results['p_fdr_awplv'].values
        min_p_plv = np.min(plv_pvals) if len(plv_pvals) > 0 else 1.0
        min_p_awplv = np.min(awplv_pvals) if len(awplv_pvals) > 0 else 1.0
        
        # Create nested 1x2 subplot for PLV and awPLV
        ...
        # Always show individual data points
        ...
        sig_marker = _sig_marker(min_p_plv)
        ax_plv.set_title(f"PLV q={min_p_plv:.3f} {sig_marker}", fontsize=11)
        ...
        sig_marker = _sig_marker(min_p_awplv)
        ax_awplv.set_title(f"awPLV q={min_p_awplv:.3f} {sig_marker}", fontsize=11)
        ...
        # Truncate long ROI names for row label
        short_name = roi_name[:30] + '...' if len(roi_name) > 30 else roi_name
        # Add ROI name as outer title using axes positions
        ...
        fig.text(mid_x, mid_y, short_name, ha='center', va='bottom', fontsize=10, 
                weight='bold', style='italic')
    
    # Caption
    fig.text(0.5, 0.02,
             r"$\bf{Figure}$: KDE density estimates of EGG-brain synchrony for top ROIs. "
    ...
    density_file = plot_dir / "roi_density_grid_top20.png"
    plt.savefig(density_file, dpi=200, bbox_inches='tight')
    plt.close()
    print(f"  ✓ Saved ROI density grid to: {density_file}")
    return density_file


def plot_roi_pooled_density(results_df, null_results, all_subjects_data, 
                             empirical_results, valid_rois, roi_labels, plot_dir):
    """
    Create a v5-style pooled density plot: empirical vs null distributions 
    pooled across all ROIs and subjects.
    
    This is the direct analogue of the v5 plot — left panel PLV, right panel awPLV.
    """
    # Collect all empirical and null values
    plv_empirical_all = results_df['plv_empirical'].values
    awplv_empirical_all = results_df['awplv_empirical'].values
    
    plv_null_all = []
    awplv_null_all = []
    for (subject, run), null in null_results.items():
        plv_null_all.extend(null['null_plvs'].flatten().tolist())
        awplv_null_all.extend(null['null_awplvs'].flatten().tolist())
    plv_null_all = np.array(plv_null_all)
    awplv_null_all = np.array(awplv_null_all)
    
    fig, (ax_plv, ax_awplv) = plt.subplots(1, 2, figsize=(16, 6))
    
    # --- PLV ---
    sns.kdeplot(plv_empirical_all, ax=ax_plv, color='blue', fill=True, alpha=0.3, 
               label=f'Empirical (n={len(plv_empirical_all)})')
    sns.kdeplot(plv_null_all, ax=ax_plv, color='blue', linestyle='--', 
               label=f'Null/Mismatch (n={len(plv_null_all)})')
    
    mean_emp = np.mean(plv_empirical_all)
    mean_null = np.mean(plv_null_all)
    ax_plv.axvline(mean_emp, color='blue', linewidth=2, alpha=0.7)
    ax_plv.axvline(mean_null, color='blue', linestyle=':', linewidth=2, alpha=0.5)
    
    from scipy.stats import mannwhitneyu
    stat, p_val = mannwhitneyu(plv_empirical_all, plv_null_all, alternative='greater')
    sig = _sig_marker(p_val)
    
    ax_plv.set_title(f'PLV: Empirical vs Null\n'
                     f'Effect={mean_emp - mean_null:.4f}, p={p_val:.4f} {sig}', fontsize=16)
    ...
    # --- awPLV ---
    sns.kdeplot(awplv_empirical_all, ax=ax_awplv, color='red', fill=True, alpha=0.3, 
               label=f'Empirical (n={len(awplv_empirical_all)})')
    sns.kdeplot(awplv_null_all, ax=ax_awplv, color='red', linestyle='--', 
               label=f'Null/Mismatch (n={len(awplv_null_all)})')
    
    mean_emp = np.mean(awplv_empirical_all)
    mean_null = np.mean(awplv_null_all)
    ax_awplv.axvline(mean_emp, color='red', linewidth=2, alpha=0.7)
    ax_awplv.axvline(mean_null, color='red', linestyle=':', linewidth=2, alpha=0.5)
    
    stat, p_val = mannwhitneyu(awplv_empirical_all, awplv_null_all, alternative='greater')
    sig = _sig_marker(p_val)
    ...
    fig.suptitle('EGG-Brain Synchrony: Pooled Across All ROIs',
                 fontsize=20, weight='bold', y=1.03)
    
    fig.text(0.5, -0.04,
             r"$\bf{Figure}$: Gaussian KDE density estimates of EGG-brain synchrony, pooled across all ROIs and subjects. "
             "Empirical (filled) vs Null-mismatch (dashed).\n"
             "PLV in blue; awPLV in red. Vertical lines show means. "
             "Stars: * p<0.05, ** p<0.01, *** p<0.001 (Mann-Whitney U one-sided test).",
             ha='center', fontsize=12)
    
    plt.tight_layout()
    pooled_file = plot_dir / "roi_pooled_density.png"
    plt.savefig(pooled_file, dpi=200, bbox_inches='tight')
    plt.close()
    print(f"  ✓ Saved pooled density plot to: {pooled_file}")
    return pooled_file


def _sig_marker(p):
    """Return significance marker string based on p-value."""
    if p < 0.001:
        return '***'
    elif p < 0.01:
        return '**'
...
def main(metadata_path=None):
    ...
    # Plot 0a: V5-style density grid for top 20 ROIs
    print("\n  Creating v5-style density plots...")
    plot_roi_density_grid(
        results_df, null_results, all_subjects_data,
        empirical_results, valid_rois, roi_labels, PLOT_DIR, n_top=20
    )
    
    # Plot 0b: Pooled density plot (all ROIs combined)
    plot_roi_pooled_density(
        results_df, null_results, all_subjects_data,
        empirical_results, valid_rois, roi_labels, PLOT_DIR
    )
    ...
    print(f"  - roi_density_grid_top20.png")
    print(f"  - roi_pooled_density.png")
